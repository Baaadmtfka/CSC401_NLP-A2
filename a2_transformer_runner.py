...
class TransformerRunner:
    ...
    def train_input_target_split(
        ...
        """
        model_inputs = target_tokens[:, :-1] # [batch_size, seq_len - 1]
        training_targets = target_tokens[:, 1:].reshape(-1) # [batch_size * (seq_len - 1)]
    ...
    def train_for_epoch(
        ...
        for i, (source, source_lens, targets) in enumerate(dataloader):
            ...
            # split
            model_inputs, training_targets = self.train_input_target_split(targets)

            # logits via the model
            logits = self.model(source, model_inputs)

            # loss via criterion
            loss = criterion(logits.view(-1, logits.size(-1)),training_targets.view(-1)) / accum_iter  # Scale the loss for gradient accumulation
            loss.backward()
            total_loss += loss.item() * accum_iter  # Undo the scaling for reporting
        ...
        avg_loss = total_loss / (len(dataloader) * targets.shape[1] - 1)
    ...
    def translate(self, input_sentence):
        ...
        source_ids = []
        for token in tokens:
            # not exist: self.dataset.source_unk_id
            source_ids.append(self.dataset.source_word2id.get(token, self.dataset.source_unk_id))

        # SOS + ... + EOS 
        source_ids = [self.dataset.source_sos_id] + source_ids + [self.dataset.source_eos_id]
        # Convert to tensor and add batch dimension
        source_tokens = torch.tensor(source_ids).unsqueeze(0)  # Shape: [1, seq_len]
        # === ============== === #
        ...
        # === Your Code Here === #
        # to list
        output_ids = hypotheses.squeeze(0).tolist()
        output_tokens = []
        for id in output_ids:
            output_tokens.append(self.dataset.target_id2word[id])
    ...
    def compute_batch_total_bleu(
        ...
        """
        batch_size = target_y_ref.shape[0]
        total_bleu_scores = [0.0] * len(n_gram_levels)

        for i in range(batch_size):
            # remove special tokens
            ref_seq = [token for token in target_y_ref[i].tolist() if token not in {sos_idx, eos_idx, pad_idx}]
            cand_seq = [token for token in target_y_cand[i].tolist() if token not in {sos_idx, eos_idx, pad_idx}]

            for j, n in enumerate(n_gram_levels):
                bleu_score = bleu_score_func(ref_seq, cand_seq, n)

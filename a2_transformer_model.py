...
class LayerNorm(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        """
        #print(x.shape)
        mu = torch.mean(x, dim=2, keepdim=True) #[batch_size, seq_len, d_model]
        sigma = torch.std(x, dim=2, keepdim=True) #[batch_size, seq_len, d_model]
        # gamma = self.gamma.repeat(x.shape[0], x.shape[1], 1)
        # beta = self.beta.repeat(x.shape[0], x.shape[1], 1)
        gamma = self.gamma #[1, d_model]
        beta = self.beta #[1, d_model]
        #print(gamma.shape)
        y = gamma * (x - mu) / (sigma + self.eps) + beta #[batch_size, seq_len, d_model]
        return y
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query: Tensor, key: Tensor, value: Tensor, mask: Tensor = None) -> Tensor:
        ...
        scores = (query @ key.transpose(2, 3)) / math.sqrt(self.d_head) #[batch_size, num_heads, query_seq_len, key_seq_len]
        #print(mask.shape)
        if mask != None:
            mask = mask.unsqueeze(1) #[batch_size, 1, query_seq_len, key_seq_len]
            #print(mask.shape, scores.shape)
            scores = scores.masked_fill(mask, float('-inf')) #[batch_size, num_heads, query_seq_len, key_seq_len] same
        self.set_attention_scores(scores)

        ###nn.LogSoftmax?
        softmax = nn.Softmax(dim=3) # softmax over key_seq_len
        softmax_attention = softmax(scores) # same shape
        dropout_attention = self.atten_dropout(softmax_attention) # same shape
        output = dropout_attention @ value # [batch_size, num_heads, query_seq_len, d_head]
    ...
    def forward(self, query: Tensor, key: Tensor = None, value: Tensor = None, mask: Tensor = None) -> Tensor:
        ...
        # attention
        query = self.q_linear(query).view(batch_size, query_seq_len, self.num_heads, self.d_head).transpose(1, 2) # [batch_size, num_heads, query_seq_len, d_head]
        if (key == None and value == None):
            attention_output = self.attention(query=query, key=query, value=query, mask=mask) # [batch_size, num_heads, query_seq_len, d_head]
        else:
            batch_size, key_seq_len, d_model = key.shape
            key = self.k_linear(key).view(batch_size, key_seq_len, self.num_heads, self.d_head).transpose(1, 2) # [batch_size, num_heads, key_seq_len, d_head]
            value = self.v_linear(value).view(batch_size, key_seq_len, self.num_heads, self.d_head).transpose(1, 2) # [batch_size, num_heads, key_seq_len, d_head]
            attention_output = self.attention(query=query, key=key, value=value, mask=mask) # [batch_size, num_heads, query_seq_len, d_head]

        # output
        output = attention_output.transpose(1, 2) # [batch_size, query_seq_len, num_heads, d_head]
        output = output.reshape(batch_size, query_seq_len, d_model) # [batch_size, query_seq_len, d_model]
        #output = attention_output.transpose(1, 2).contiguous().view(batch_size, query_seq_len, d_model)
        output = self.dropout(self.out_linear(output)) # same shape
        return output
...
class FeedForwardLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        #print(x.shape)
        x = self.dropout1(self.f(self.w_1(x))) #[batch_size, seq_len, d_model]
        x = self.dropout2(self.w_2(x)) #[batch_size, seq_len, d_model]
        #print(x.shape)
        return x
...
class TransformerEncoderLayer(nn.Module):
    ...
    def pre_layer_norm_forward(self, x: Tensor, mask: Tensor) -> Tensor:
        ...
        """
        x = self.self_attn.forward(query=self.ln1.forward(x), mask=mask) + x
        x = self.ff.forward(self.ln2.forward(x)) + x
        return x

    def post_layer_norm_forward(self, x: Tensor, mask: Tensor) -> Tensor:
        x = self.self_attn.forward(query=x, mask=mask) + x
...
class TransformerDecoderLayer(nn.Module):
    ...
    def pre_layer_norm_forward(self, x: Tensor, mask: Tensor, src_x: Tensor, src_mask: Tensor) -> Tensor:
        x = self.self_attn.forward(query=self.ln1.forward(x), mask=mask) + x
        x = self.cross_attn.forward(query=self.ln2.forward(x), key=src_x, value=src_x, mask=src_mask) + x
        x = self.ff.forward(self.ln3.forward(x)) + x
        return x

    def post_layer_norm_forward(self, x: Tensor, mask: Tensor, src_x: Tensor, src_mask: Tensor) -> Tensor:
        x = self.self_attn.forward(query=x, mask=mask) + x
        x = self.ln1.forward(x)
        x = self.cross_attn.forward(query=x, key=src_x, value=src_x, mask=src_mask) + x
        x = self.ln2.forward(x)
        x = self.ff.forward(x) + x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor, src_x: Tensor, src_mask: Tensor,
        ...
        if self.is_pre_layer_norm:
            x = self.norm(x) # [batch_size, query_seq_len, d_model]
        # project to logits
        x = self.proj.forward(x) # [batch_size, query_seq_len, vocab_size]
        #print("Decoder pre-logSoftMax x.shape:", x.shape)
        if normalize_logits:
            log_softmax = nn.LogSoftmax(dim=2) # logsoftmax over vocab_size
            x = log_softmax.forward(x)
        return x
...
class TransformerEncoderDecoder(nn.Module):
    ...
    def create_pad_mask(self, tokens: Tensor) -> Tensor:
        ...
        """
        pad_mask = tokens.eq(self.padding_idx).unsqueeze(1) # [batch_size, 1, seq_len]
    ...
    def create_causal_mask(tokens: Tensor) -> Tensor:
        ...
        mask = torch.full((seq_len, seq_len), True) # [seq_len, seq_len]
        causal_mask = torch.triu(mask, diagonal=1).unsqueeze(0) # [1, seq_len, seq_len]
    ...
    def forward(self, src: Tensor, tgt: Tensor, normalize_logits: bool = False) -> Tensor:
        ...
        """
        # embeddings
        src_embeddings = self.get_src_embeddings(src) # [batch_size, src_seq_len, d_model]
        tgt_embeddings = self.get_tgt_embeddings(tgt) # [batch_size, tgt_seq_len, d_model]

        # masks
        # src mask
        src_mask = self.create_pad_mask(src) # [batch_size, 1, src_seq_len]
        src_mask = src_mask.expand(-1, src.shape[1], -1) # [batch_size, src_seq_len, src_seq_len]
        #print("src_mask",src_mask.shape)
        # tgt mask
        tgt_src_mask = self.create_pad_mask(src).expand(-1, tgt.size(1), -1)
        tgt_pad_mask = self.create_pad_mask(tgt) # [batch_size, 1, tgt_seq_len]
        tgt_pad_mask = tgt_pad_mask.expand(-1, tgt.shape[1], -1) # [batch_size, tgt_seq_len, tgt_seq_len]
        tgt_causal_mask = self.create_causal_mask(tgt) # [1, tgt_seq_len, tgt_seq_len]
        tgt_causal_mask = tgt_causal_mask.expand(tgt.shape[0], -1, -1) # [batch_size, tgt_seq_len, tgt_seq_len]
        tgt_mask = tgt_pad_mask | tgt_causal_mask # [batch_size, tgt_seq_len, tgt_seq_len]
        #print("tgt_mask",tgt_mask.shape)

        # encoder
        encoder_output = self.encoder.forward(src_embeddings, src_mask)
        # decoder
        #decoder_output = self.decoder.forward(tgt_embeddings, tgt_mask, src_embeddings, src_mask, normalize_logits)
        decoder_output = self.decoder.forward(tgt_embeddings, tgt_mask, encoder_output, tgt_src_mask, normalize_logits)
    ...
    def concatenate_generation_sequence(tgt_generation: Tensor, next_token: Tensor) -> Tensor:
        ...
        """
        return torch.cat([tgt_generation, next_token], dim=-1).to(tgt_generation.device)
    ...
    def greedy_decode(self, src: Tensor, target_sos: int, target_eos: int, max_len: int = 100) -> Tensor:
        ...
        # gen
        current_gen = self.initialize_generation_sequence(src, target_sos) # [batch_size, 1]
        while not self.all_finished(current_gen, target_eos, max_len):
            # tgt embeddings
            tgt_embeddings = self.get_tgt_embeddings(current_gen) # [batch_size, tgt_seq_len, d_model]
            # tgt mask
            tgt_src_mask = self.create_pad_mask(src).expand(-1, current_gen.size(1), -1)
            tgt_pad_mask = self.create_pad_mask(current_gen) # [batch_size, 1, tgt_seq_len]
            tgt_pad_mask = tgt_pad_mask.expand(-1, current_gen.shape[1], -1) # [batch_size, tgt_seq_len, tgt_seq_len]
            tgt_causal_mask = self.create_causal_mask(current_gen) # [1, tgt_seq_len, tgt_seq_len]
            tgt_causal_mask = tgt_causal_mask.expand(current_gen.shape[0], -1, -1) # [batch_size, tgt_seq_len, tgt_seq_len]
            tgt_mask = tgt_pad_mask | tgt_causal_mask # [batch_size, tgt_seq_len, tgt_seq_len]
            # decoder
            decoder_output = self.decoder.forward(tgt_embeddings, tgt_mask, encoder_output, tgt_src_mask, normalize_logits=True) # [batch_size, tgt_seq_len, vocab_size]
            decoder_output = decoder_output[:, -1, :] # [batch_size, 1], only need last token
            decoder_output_token = torch.argmax(decoder_output, dim=1, keepdim=True) # [batch_size, 1]
            current_gen = self.concatenate_generation_sequence(current_gen, decoder_output_token)
        gen = self.pad_generation_sequence(current_gen, target_eos)
        return gen
    ...
    def expand_encoder_for_beam_search(src_x: Tensor, src_mask: Tensor, k: int) -> Tuple[Tensor, Tensor]:
        ...
        expanded_src_x = src_x.unsqueeze(1).expand(-1, k, -1, -1) # [batch_size, k, src_seq_len, d_model]
        expanded_src_x = expanded_src_x.reshape(batch_size * k, src_x.shape[1], src_x.shape[2]) # [batch_size * k, src_seq_len, d_model]
        
        expanded_src_mask = src_mask.unsqueeze(1).expand(-1, k, -1, -1) # [batch_size, k, 1, src_seq_len]
        expanded_src_mask = expanded_src_mask.reshape(batch_size * k, src_mask.shape[1], src_mask.shape[2]) # [batch_size * k, 1, src_seq_len]
    ...
    def repeat_and_reshape_for_beam_search(t: Tensor, k: int, expan: int, batch_size: int) -> Tensor:
        ...
        t = t.view(batch_size, k, cur_len) # [batch_size, k, cur_len]
        t = t.unsqueeze(2).expand(-1, -1, expan, -1)  # [batch_size, k, expan, cur_len]
        t = t.reshape(batch_size, k * expan, cur_len) # [batch_size, k * expan, cur_len]
    ...
    def initialize_beams_for_beam_search(
        ...
        src_embeddings = self.get_src_embeddings(src)  # [batch_size, src_seq_len, d_model]
        src_mask = self.create_pad_mask(src)  # [batch_size, 1, src_seq_len]
        encoder_output = self.encoder(src_embeddings, src_mask)  # [batch_size, src_seq_len, d_model]

        # gen
        current_gen = self.initialize_generation_sequence(src, target_sos)  # [batch_size, 1]
        tgt_embeddings = self.get_tgt_embeddings(current_gen)  # [batch_size, 1, d_model]
        # causal without pad mask
        tgt_mask = self.create_causal_mask(current_gen)  # [1, 1, 1]
        decoder_output = self.decoder(tgt_embeddings, tgt_mask, encoder_output, src_mask, normalize_logits=True)  # [batch_size, 1, vocab_size]
        # eos with prob. -inf
        decoder_output[:, :, target_eos] = float('-inf')
        # topk
        log_probs, top_k_predictions = torch.topk(decoder_output[:, -1, :], k, dim=1)  # [batch_size, k]
        # reshape 
        log_probs = log_probs.reshape(-1, 1)  # [batch_size * k, 1]
        top_k_predictions = top_k_predictions.reshape(-1, 1)  # [batch_size * k, 1]

        # init beam tensor
        batch_size = src.shape[0]
        current_gen = current_gen.unsqueeze(1).expand(batch_size, k, -1).reshape(-1, 1)  # [batch_size * k, 1]
        current_gen = self.concatenate_generation_sequence(current_gen, top_k_predictions)  # [batch_size * k, 2]
        sos_log_probs = torch.zeros(batch_size * k, 1, device=log_probs.device)  # [batch_size * k, 1]
        log_probs = torch.cat([sos_log_probs, log_probs], dim=-1) # [batch_size * k, 2]

        # expand encoder outputs
        expanded_encoder_output, expanded_src_mask = self.expand_encoder_for_beam_search(encoder_output, src_mask, k)

        return current_gen, log_probs, expanded_encoder_output, expanded_src_mask
    ...
    def score_sequence_for_beam_search(
        ...
        """
        padded_seq_log_probs = self.pad_generation_sequence(seq_log_probs, self.padding_idx)
        scores = padded_seq_log_probs.sum(dim=-1)  # [...]

        return seq_log_probs, scores
